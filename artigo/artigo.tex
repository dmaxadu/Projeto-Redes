...
\author{[Arlene Pelenda Julienne]\inst{1}, [Daniel Machado da Conceição]\inst{1}, [Leonardo de Melo Soares]\inst{1}, [Sylvio Heltt]\inst{1}, [Victor Pereira de Lima]\inst{1}}
...
\begin{abstract}
  ...
  coup of February 2021 (cyber attacks and route manipulation, in relation to Myanmar civil war). 
  Using public data from RIPE RIS, RouteViews, IODA/CAIDA, OONI, and Cloudflare Radar, we
...
\begin{resumo}
  ...
  em Myanmar (fevereiro de 2021), enquadrando o problema de
  ataques cibernéticos e manipulação de rotas em relação à questão
  da guerra civil em Myanmar. Usando dados públicos do RIPE RIS,
...
Este trabalho investiga a \textbf{ataques cibernéticos e
manipulação de rotas BGP} em relação à \textbf{
...
Sobre os \textbf{ataques cibernéticos e manipulação de rotas},
...
\textbf{O BGP} é o protocolo de roteamento inter-domínio da Internet. Cada organização
...
\begin{itemize}
  ...
  \item \textbf{AS132748} --- Ooredoo Myanmar (rebatizada U9 em 2025 após
    venda para Nine Communications de Singapura)~\cite{dcd:2025};
  \item \textbf{AS132167} --- Atom Myanmar (ex-Telenor, vendida para joint
    venture ligada ao exército em 2022)~\cite{dcd:2025}.
\end{itemize}
...
\caption{Volume de retiradas de prefixos BGP por hora para os quatro ISPs
  ...
  \textbf{Dados agregados e processados a partir de fontes primárias
  (RouteViews MRT) e secundárias~\cite{ooni:multiperspective,netblocks:2021}.}}
...
janeiro (linha de base) e durante os curfews revela quedas de visibilidade
próximas a 0\% durante as janelas noturnas. Por exemplo, o AS9988 (MPT)
apresentou queda de 100\% dos prefixos anunciados durante os curfews, enquanto
o AS136168 (Mytel) manteve cerca de 15\% dos prefixos visíveis, sugerindo
que a ordem de apagão pode ter sido aplicada de forma diferenciada entre
operadoras.


\subsection{O Hijack BGP do Twitter (M5)}
...
Os dados do RouteViews e do Isolario registram as trajetórias de anúncio no
momento do início do sequestro de rotas (M5), detalhados na Tabela~\ref{tab:hijack-paths}.

\begin{table}[ht]
\centering
\caption{Updates BGP capturados registrando o início do hijack (05/02/2021)}
\label{tab:hijack-paths}
\begin{tabular}{@{}lllll@{}}
\toprule
\textbf{Data} & \textbf{Horário (UTC)} & \textbf{Prefixo Alvo} & \textbf{Peer ASN} & \textbf{Caminho de AS (AS\_PATH)} \\
\midrule
05/02/21 & 15:51:17 & \texttt{104.244.42.0/24} & 37989 & 56300 132132 136168 \\
05/02/21 & 15:51:21 & \texttt{104.244.42.0/24} & 61292 & 136168 \\
05/02/21 & 15:51:40 & \texttt{104.244.42.0/24} & 37989 & 4844 136168 \\
05/02/21 & 15:51:13 & \texttt{104.244.42.0/24} & 18106 & 136168 \\
05/02/21 & 15:51:21 & \texttt{104.244.42.0/24} & 23673 & 136168 \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Nota sobre M6}: embora a ausência de ROA do Twitter tenha facilitado
a propagação do hijack, a métrica M6 deste trabalho foca na cobertura RPKI
dos ASes de Myanmar. Conforme dados do RIPEstat em fevereiro de 2021,
nenhum dos quatro ASes monitorados (AS9988, AS136168, AS132748, AS132167)
possuía ROAs publicados para seus prefixos. Isso significa que a cobertura
RPKI de Myanmar era \textbf{0\%} na época, tornando o país inteiramente
vulnerável a ataques de origem de rota, tanto internos quanto externos.
Dois dias depois, em 7 de fevereiro, o AS136168 repetiu a técnica com o
prefixo \texttt{1.32.58.0/24}, desta vez aceito apenas por três ASes ---
evidência de que alguns operadores atualizaram suas políticas após o primeiro
incidente, mas a vulnerabilidade estrutural permaneceu.
...
A análise via CAIDA AS Relationships revelou os upstreams dominantes dos
ASes de Myanmar. A Tabela~\ref{tab:upstreams} lista os upstreams conhecidos
do AS136168 (Mytel) segundo o RIPEstat na época do incidente, incluindo o
AS7552 (Viettel), que é a controladora da Mytel e atua como seu principal
provedor de trânsito internacional, embora nem sempre apareça nas rotas
públicas devido a acordos de trânsito privado.
...
\midrule
AS7552   & Viettel (controladora) & Vietnã \\
...
Para \textbf{M4 (concentração top-5)}, observamos que o AS6939 (Hurricane
Electric) respondeu por \textbf{62\%} dos caminhos observados para o AS136168,
enquanto os demais upstreams (incluindo o AS7552) dividiram os 38\% restantes.
Essa alta concentração em um único AS americano demonstra forte dependência
de trânsito internacional, o que torna a conectividade do país vulnerável a
sanções ou pressões geopolíticas sobre provedores específicos.
...
Este trabalho analisou a manipulação de rotas BGP em
relação à questão geopolítica da guerra civil em Myanmar utilizando
...
de rota como instrumento de censura; (3) a cobertura RPKI dos ASes de Myanmar
era 0\% na época, agravando a vulnerabilidade do país a ataques de roteamento;
e (4) o sinal BGP antecipou reportagens jornalísticas em 2 a 15 minutos,
